...
class ShiftDetail extends StatefulWidget {
  ...
  ShiftDetail({Key? key, @required this.FBillNo}) : super(key: key);

  @override
  _RetrievalDetailState createState() => _RetrievalDetailState(FBillNo);
}

class _RetrievalDetailState extends State<ShiftDetail> {
    ...
    if (this.organizationsNumber1 != null) {
      userMap['FilterString'] =
          "FForbidStatus = 'A' and FDocumentStatus = 'C' and FUseOrgId.FNumber=" +
              this.organizationsNumber1.toString();
    } else {
  ...
  getStockListT() async {
    ...
    if (this.organizationsNumber2 != null) {
      userMap['FilterString'] =
          "FForbidStatus = 'A' and FDocumentStatus = 'C' and FUseOrgId.FNumber=" +
              this.organizationsNumber2.toString();
    } else {
      userMap['FilterString'] = "FForbidStatus = 'A' and FDocumentStatus = 'C'";
    }
    Map<String, dynamic> dataMap = Map();
  ...
  getMaterialList(barcodeData, code, fsn, fProduceDate, fExpiryDate, fBatchNo,
      fLoc, fIsOpenLocation) async {
    Map<String, dynamic> userMap = Map();
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    var menuData = sharedPreferences.getString('MenuPermissions');
    var deptData = jsonDecode(menuData)[0];
        ...
        for (var value in materialDate) {
          ...
          arr.add({
            ...
            "value": {
              ...
              "scanCode": [
                barCodeScan[0].toString() + "-" + barcodeNum.toString()
              ],
              "codeList": []
            }
          });
          ...
          arr.add({
            ...
            "isHide": false,
            "value": {
              "label": barcodeData[0][17],
              "value": barcodeData[0][17],
              "hide": value[16]
            }
          });
          arr.add({
          ...
          arr.add({
            ...
            "value": {
              ...
              "remainder": "0",
              "representativeQuantity": barCodeScan[4].toString()
            }
          });
          arr.add({
            "title": "生产日期",
        ...
        if (element[3]['value']['value'] != '0' &&
          ...
          if (element[7]['value']['hide']) {
            ...
            if (stockRes.length > 0) {
              ...
              for (var dimension in postionList) {
                FEntityItem['FSrcStockLocId']
                    ["FSRCSTOCKLOCID__" + stockRes[positonIndex][0]] = {
                  "FNumber": dimension
                };
                positonIndex++;
              }
          ...
          if (element[9]['value']['hide']) {
            ...
            if (stockRes.length > 0) {
              ...
              for (var dimension in postionList) {
                FEntityItem['FDestStockLocId']
                    ["FDESTSTOCKLOCID__" + stockRes[positonIndex][0]] = {
                  "FNumber": dimension
                };
                positonIndex++;
              }
                  ...
                  Column(
                    children: [
                      Container(
                        color: Colors.white,
                        child: ListTile(
                          title: Text("调入组织：$organizationsName2"),
                        ),
                      ),
                      divider,
                    ],
